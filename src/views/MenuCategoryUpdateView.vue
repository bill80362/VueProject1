...
onMounted(async () => {
  ...
  if (route.params.id !== "0") {
    //修改
    DataApi.MenuCategoryList.forEach((item, index, array) => {
      if (route.params.id === item.MenuCategoryID) {
        Data.value = item;
        Data.value.ID = item.MenuCategoryID;
      }
...
const handleSubmit = async () => {
  await DataApi.apiMenuCategoryUpdateCreate(Data.value);
  handleBack();
}
